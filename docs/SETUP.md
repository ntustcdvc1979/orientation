...
| <kbd>←</kbd> | 上一步 |
| <kbd>Esc</kbd> | 跳關選單（全部 52 個畫面，點一下直接跳） |
| <kbd>F</kbd> | 全螢幕 |
...
| 任務一～三 | 三題都是從新生的角度出發（外食一週、熬夜爆肝、一個人在外地）。出情境 → <kbd>T</kbd> 開始計時 → 手上有解的人舉卡 → 隨機請 2–3 位念出卡片上寫的功效 → <kbd>→</kbd> 公布解答。 |
...
| 志工照片牆 | 接著把社團實際做過的服務照片鋪滿整個畫面，讓剛剛那段話有畫面對得起來。 |
| 💎 開箱 標題 | 「把箱子打開之前，先站在一起。」 |
...
把照片放進 `media/photos/`，命名 `01.jpg` ~ `07.jpg`，
清單在 [`assets/photos-data.js`](../assets/photos-data.js)。要換張數或檔名就改那一份。

每一筆可以加 `span` 決定版面：`big`（兩欄兩列，當主視覺）、
`tall`（**直式照片一定要加**，不然人臉會被裁掉）、`wide`（兩欄寬）、不填就是一般大小。

檔案還沒放進去也不會壞——載不到的那格會自己消失，全部都載不到就顯示一行提示。
