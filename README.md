...
| 🤝 | **實踐服務** | 三隻猴子：一個只能比、一個只能說、一個閉著眼睛把答案畫出來。收尾放仲由夫子那段話，再把社團做過的服務照片鋪滿投影幕，然後是賑災現場與海外義診交流 |
| 💎 | **開寶箱** | 全體大合照，然後打開箱子——裡面沒有金幣，只有剛剛那張照片 |
| ⛵ | 終章 | 「寶藏就在你身上」，法船啟航影片（自動重播）。結束卡放崇德禮讚，附傳送門 QR、六張近期活動與這學期行事曆 |
...
沒設定 Firebase 也不會壞：投影幕會自動退到本機模式，鍵盤照樣跑完全部 55 個畫面，
只是手機端不會同步。
...
assets/
  ...
  cards-data.js                28 種食材卡與 3 個情境的參考解答
  monkeys-data.js              三隻猴子的角色與題目（範例太陽＋甜甜圈、淨灘）
  photos-data.js               三面照片牆的清單（開伙、志工服務、海外義診）
  qrcode.js                    自製 QR 產生器，不依賴任何外部服務
  quiz.mp3 cook.mp3            各關的配樂，跨關自動淡出淡入
  volunteer.mp3
  logo1.webp logo2.webp        兩顆社徽
  signup-qr.webp               傳送門 QR（從 2026問卷.png 裁出來的）
  donut.webp 等六張             最後一頁的近期活動宣傳
