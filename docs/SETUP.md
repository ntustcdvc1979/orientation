...
| <kbd>←</kbd> | 上一步 |
| <kbd>Esc</kbd> | 跳關選單（全部 55 個畫面，點一下直接跳） |
| <kbd>F</kbd> | 全螢幕 |
...
出題順序是**範例「太陽」→ 甜甜圈 → 淨灘**，共兩題正式題。
到最後一題再按 <kbd>→</kbd> 不會繞回去，畫面會提示按 <kbd>Shift</kbd>+<kbd>→</kbd> 繼續。
題目要改就動 [`assets/monkeys-data.js`](../assets/monkeys-data.js)。

### 配樂

一關一首，跨關的時候自動淡出淡入，音量壓在 0.3，主持人講話蓋得過去：

| 關卡 | 曲子 |
|---|---|
| 🧭 認識自己 | `assets/quiz.mp3` |
...
控制列上的 🔊 按鈕滑過去會顯示現在放的是哪一首，按下去（或按 <kbd>M</kbd>）可以靜音。

結尾的法船影片設成**自動重播**，可以一直放著當背景。
...
| 情境一～三 | 三題都是從新生的角度出發（外食一週、熬夜爆肝、一個人在外地）。出情境 → <kbd>T</kbd> 開始計時 → 手上有解的人舉卡 → 隨機請 2–3 位念出卡片上寫的功效 → <kbd>→</kbd> 公布解答。 |
| 終極任務 | 為一個人做一餐，六大類至少四類。每個上場的人要說「我這張卡是為他哪一件事準備的」。收尾講白米／豆腐／豆漿那段。 |
| 🍲 第二段完成 | 收尾：「會為別人多想這一步的人，你剛剛都認識了。」 |
| 開伙的樣子 | 六張伙食團的照片鋪滿畫面：「這就是我們平常開伙的樣子。」 |
| 🤝 第三段 標題 | 「知道自己是誰、身邊有誰之後，剩下的就是真的去做。」 |
| 三隻猴子 規則 | 三個人一組排成一直線，說明三種猴子。**重點是答案不用講的，是猜題猴閉著眼睛畫出來**，最後拿畫跟原題比。記得先發紙筆。 |
| 三隻猴子 出題 | 先走範例題「太陽」讓大家看懂規則，再進甜甜圈、淨灘兩題。比劃猴面對投影幕，另外兩位背對。 |
| 🤝 第三段完成 | 收尾放仲由夫子那段話：「唯有為眾生付出，為眾生服務，才能夠千年甚至萬年被傳頌。」 |
| 志工照片牆 | 把社團實際做過的服務照片鋪滿整個畫面，讓剛剛那段話有畫面對得起來。 |
| 賑災現場 | 「賑災在哪，我們就在哪」。醫療關懷隊帶著血壓計、血氧機出隊的那組照片。 |
| 海外義診交流 | 再翻一頁：服務不只在台灣。講義診、講交流、講國際視野。 |
| 💎 開寶箱 標題 | 「把箱子打開之前，先站在一起。」 |
| 大合照 | 按 <kbd>T</kbd> 跑 3・2・1，攝影同學抓最後那張 📸。 |
| 💎 箱子打開了 | **收線**：「裡面沒有金幣，只有剛剛那張照片。」 |
| 終章 | 「寶藏就在你身上。」講完按 <kbd>→</kbd> 播影片。 |
| 影片 | 會自動重播，可以一直放著；要離開按 <kbd>→</kbd> 到結束卡。 |
| 結束卡 | 崇德禮讚會自己放起來。畫面上有傳送門 QR、六張近期活動宣傳與這學期的行事曆，可以停在這裡讓大家慢慢拍。 |
...
清單在 [`assets/photos-data.js`](../assets/photos-data.js)，分成三組：
...
每一組可以自己設 `eyebrow`（小標）、`title`（大標）、`lede`（副標，可留空）。

**圖檔一律用 webp**，放在 `media/photos/`。原始 jpg 放在 `media/photos/original/`，
那個資料夾在 `.gitignore` 裡不會進版本庫。要加新照片的話：

```bash
cwebp -q 80 -metadata none 新照片.jpg -o media/photos/新照片.webp
```

長邊超過 1600px 的加上 `-resize 1600 0`（直式照片是 `-resize 0 1600`）。

版面是自動的：同一列的照片等高、寬度照原始比例，程式會算出「排得下的最大高度」，
所以**每一張都完整呈現，不會被裁到**，直式橫式混著放也沒問題。
張數改幾張都排得出來，檔案沒放進去的那格會自己消失。
